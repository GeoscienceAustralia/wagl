...
!   Calculates brdf corrected and terrain corrected surface reflectance.
...
!   input parameters
    ...
    real*4 ref_lm(nrow, ncol) ! input lambertian reflectance

!   input and output arrays
    real*4 ref_brdf(nrow, ncol) ! brdf corrected reflectance (inout)
    real*4 ref_terrain(nrow, ncol) ! terrain illuminated corrected reflectance (inout)


!f2py depend(nrow, ncol), ref_lm, ref_brdf, ref_terrain
!f2py depend(nrow, ncol), shadow_mask
...
!f2py depend(nrow, ncol), rela_slope, s_mod, fv, fs, ts
!f2py depend(nrow, ncol), edir_h, edif_h
!f2py intent(in) rori, norm_1, norm_2, ref_adj, no_data, norm_solar_zenith
!f2py intent(in) shadow_mask, solar_angle,
!f2py intent(in) sazi_angle, view_angle, rela_angle, slope_angle, aspect_angle
!f2py intent(in) it_angle, et_angle, rela_slope, s_mod, fv, fs, ts, edir_h, edif_h
!f2py intent(in) ref_lm
!f2py intent(inout) ref_brdf, ref_terrain
...
!           ignore anything with too low a value
            if (ref_lm(i, j) .gt. 0.0001) then
...
!
                ...
                a_eqf = (1 - aa_flat) * s_mod(i, j) * (1 - s_mod(i, j) * &
                        ref_lm(i, j))
                b_eqf = aa_flat + ref_lm(i, j) * (1 - aa_flat) * s_mod(i, j)
                c_eqf = -ref_lm(i, j)
...
!                   following line appears to be unused so ignore for now
!                    ref_brdfrealf = ann_f * ref_barf / aa_white
                    ref_brdf(i, j) = ref_barf * fnn / aa_white
            endif
!-------------------------------------------------------------------
...
!               radiation" Hay model
                ...
                rth = (rori - s_mod(i, j) * ref_lm(i, j)) / (1 - s_mod(i, j) * &
                      ref_lm(i, j))
...
!               calculate black sky albedo for view angle
                ...
                a_eqs = (rtotal - aa_slope) * s_mod(i, j) * &
                        (1 - s_mod(i, j) * ref_lm(i, j))
                b_eqs = aa_slope + ref_lm(i, j) * (1 - aa_slope) * s_mod(i, j)
                c_eqs = -ref_lm(i, j)
                ...
                ref_terrain(i, j) = ref_bars * fnn / aa_white
