...
class WorldView2Acquisition(Acquisition):
    ...
    def __init__(self, pathname, uri, acquisition_datetime, band_name='BAND-C',
                 band_id='1', metadata=None):
        super().__init__(pathname, uri, acquisition_datetime, band_name=band_name,
                         band_id=band_id, metadata=metadata)
...
class WorldView2MultiAcquisition(WorldView2Acquisition):
    ...
    band_names = ['BAND-{}'.format(name)
                  for name in ["C", "B", "G", "Y", "R", "RE", "N", "N2"]]
    sensor_id = 'MUL'
    ...
    def radiance_data(self, window=None, out_no_data=-999, esun=None):
        ...
        radiance = self.gain * data * (self.abs_cal_factor / self.effective_bandwidth) + self.offset
