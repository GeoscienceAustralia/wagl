...
def run(level1, vertices='(5, 5)', workflow='standard', method='linear',
        pixel_quality=False, outdir=None, logdir=None, env=None, nodes=1,
        project=None, queue='normal', walltime='48:00:00', buffer_distance=8000,
        email='your.name@something.com', local_scheduler=False, dsh=False,
        test=False, singlefile=False, task=None, ncpus=48, workers=30,
        memory=192):
    """Base level program."""
    ...
    if dsh:
        _submit_dsh(scattered, options, env, batchid, batch_logdir,
                    batch_outdir, pbs_resources, test, workers)
    else:
...
def _parser():
    ...
    parser.add_argument("--buffer-distance", default=8000, type=float,
                        help=("The distance in units to buffer an image's "
                              "extents by."))
    parser.add_argument("--ncpus", default=48, type=int,
                        help="The number of cpus per node to request.")
    parser.add_argument("--workers", default=30, type=int,
                        help="The number of workers per node to request.")
    parser.add_argument("--memory", default=192, type=int,
...
def main():
    ...
    run(args.level1_list, args.vertices, args.workflow, args.method,
        ...
        args.project, args.queue, args.walltime, args.buffer_distance,
        args.email, args.local_scheduler, args.dsh, args.test, args.singlefile,
        args.task, args.ncpus, args.workers, args.memory)
