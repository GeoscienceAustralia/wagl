...
def run_modtran(
    ...
    if out_group is None:
        fid = h5py.File("atmospheric-results.h5", "w", driver="core", backing_store=False)
    else:
        fid = out_group
    ...
    base_path = ppjoin(GroupName.ATMOSPHERIC_RESULTS_GRP.value, POINT_FMT.format(p=point))
    ...
    for albedo in albedos:
        ...
        json_mod_infile = pjoin(
            workpath, "".join([POINT_ALBEDO_FMT.format(p=point, a=albedo.value), ".json"])
        )
...
def read_spectral_response(fname, spectral_range=None):
    ...
    idx = ids[-1]
    data = numpy.array([line.split("  ") for line in lines[idx + 1 :]], dtype="float")
    df = pd.DataFrame(
        {"band_name": lines[idx], "wavelength": data[:, 0], "response": data[:, 1]}
    )
    response[lines[idx]] = df
