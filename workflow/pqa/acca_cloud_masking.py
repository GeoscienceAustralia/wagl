...
def acca(reflectance_stack, thermal_array, potential_cloud_array, pq_const, aux_data={}):
    """
...
#"""
    ...
    if cloud.sum() > 0:
        ...
        logging.debug('Mean temperature: %s', numpy.mean(thermal_array[cloud], dtype='float'))
        ...
        if ((cold_cloud_pop > pq_const.acca_coldCloud_pop) and (desert_index > pq_const.acca_desertIndex)
            ...
            r_cloud = acca_2nd_pass(cloud_mask=cloud, ambiguous_array=ambiguous_array, \
                thermal_array=thermal_array, mean_cloud_temp=cold_cloud_mean, \
...
def calc_acca_cloud_mask(nbar_array, kelvin_array, pq_const, contiguity_mask, aux_data={}):
    """
    ...
    :return:
        ...
        Note: Any caller-supplied aux_data dict will be updated
    """
    if len(nbar_array) == 0: return None
