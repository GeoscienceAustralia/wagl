...
def slope_aspect_arrays(acquisition, dsm_fname, margins, slope_out_fname,
    ...
    slope_aspect(ncol, nrow, cols, rows, dresx, dresy, spheroid, alat, is_utm,
                 dsm_subset, slope.transpose(), aspect.transpose())

    # Output the results
    write_img(slope, slope_out_fname, fmt='GTiff', geobox=geobox, nodata=-999,
              compress='deflate', options={'zlevel': 1})
    write_img(aspect, aspect_out_fname, fmt='GTiff', geobox=geobox,
              nodata=-999, compress='deflate', options={'zlevel': 1})
