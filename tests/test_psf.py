...
class TestPointSpreadFunction(unittest.TestCase):
    ...
    def test_computefwhm(self):
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_8"], np.arange(251) * 10.0, 10.0),
            467.444982995936,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_7"], np.arange(251) * 10.0, 10.0),
            746.22377909361,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_6"], np.arange(251) * 10.0, 10.0),
            742.7550338823073,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_5"], np.arange(251) * 10.0, 10.0),
            544.4609683735308,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_4"], np.arange(251) * 10.0, 10.0),
            465.13088437672735,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_3"], np.arange(251) * 10.0, 10.0),
            468.07414661500195,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_2"], np.arange(251) * 10.0, 10.0),
            469.7774722852594,
        )
        npt.assert_almost_equal(
            compute_fwhm(self.psf_data["Band_1"], np.arange(251) * 10.0, 10.0),
            473.56584102060157,
        )

    def test_computefiltermatrix(self):
        kernel_matrix = compute_filter_matrix(
            self.psf_data["Band_1"], 30.0, 30.0, 40, 10.0
        )
