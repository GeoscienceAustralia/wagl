        ...
        if subprocess:
            ...
            if len(module_list) == 1: # Only one sub-module specified
                if module in modtran_config.FULL_COORD_LIST: # Coordinator only specified
                    ...
                    raise Exception('Invalid coordinator or albedo specified: %s' % repr(module))
                for subsubprocess in _subsubprocess_list:
                    run_flux(subsubprocess[0], subsubprocess[1], sublogger, CONFIG.work_path, CONFIG.BIN_DIR)
            else: # Single run specified by both coord and albedo
                assert ((module_list[0] in modtran_config.FULL_COORD_LIST and module_list[1] in modtran_config.FULL_ALBEDO_LIST) or
                        (module_list[1] in modtran_config.FULL_COORD_LIST and module_list[0] in modtran_config.FULL_ALBEDO_LIST)), 'Invalid coordinator or albedo specified'
                run_flux(module_list[0], module_list[1], sublogger, CONFIG.work_path, CONFIG.BIN_DIR)
