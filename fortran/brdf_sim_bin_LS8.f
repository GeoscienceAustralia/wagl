...
!DEC$ ATTRIBUTES FORCEINLINE :: RL_brdf
        ...
        INTEGER*2 iref_brdf(NCDIM), iref_lm(NCDIM)
        INTEGER*2 dn_1(NCDIM)
        INTEGER*4 dn(NCDIM)
...
c       open image file
     ...
     &    access='direct',form='unformatted',recl=ncol*2)
...
c        ULA -- TOA reflectance is not written to disk
c        open(52,file=fname, access='direct',recl=2*ncol)

c       open output for Lambertian reflectance
        call GETARG(14, fname)
c        ULA -- Lambertian reflectance is not written to disk
c        open(53,file=fname, access='direct',recl=2*ncol)
...
c               calculate black sky albedo for solar angle
...
c          ULA -- TOA and Lambertian reflectance are not written to disk
c          write(52,rec=i)(iref_top(j),j=1,ncol)
c          write(53,rec=i)(iref_lm(j),j=1,ncol)
