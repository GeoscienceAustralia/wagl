...
c       program to read modtran *.flx data from transmission mode
c       program is wriiten by Fuqin Li in 2002 and modified in 2008
c       command line read_flx_ga_trans <*.flx> <filter_input>
c                               <output_*.dir>
...
c            write(11,*)i,j,res(j,i)
          enddo
        enddo
        write(9,*)'band   ','diffuse   ','direct   ','diffusetop  '
     &    ,'directtop   ','transmittance'
        do j=1,nband
        edo_band(j)=(res(2600,j)*edo(2600,1)+res(350,j)*edo(350,1))/2
        esun_band(j)=(res(2600,j)*esun(2600,1)+res(350,j)*esun(350,1))/2
        edo_band_top(j)=(res(2600,j)*edo(2600,inu)+res(350,j)
     &   *edo(350,inu))/2
     ...
     &   *esun(350,inu))/2
        ...
        do i=2499,351,-1
          ...
          edo_band_top(j)=edo_band_top(j)+edo(i,inu)*res(i,j)
        ...
        edo_band_top(j)=edo_band_top(j)/res_sum(j)
        esun_band_top(j)=esun_band_top(j)/res_sum(j)
        write(9,*)j,edo_band(j),esun_band(j),edo_band_top(j),
     &    esun_band_top(j),(edo_band(j)+esun_band(j))/
     &    (edo_band_top(j)+esun_band_top(j))
        enddo
        stop
      end
