/*****************************************************************
 * Extract the Ozone value from a 1 deg square Ozone based on lat/long
 *
 *    unsigned integer value
 *
 * @author: Paul Gardner  Created on: 04/06/2008
 *
 *    command line: extractOzone Ozone Lat Long
 *
 *****************************************************************
 */
...
#include "extractOzone.h"

char OzoneFile[200], LatString[20], LongString[20];
float Lat, Long;

float readOzoneValue(Gtif *inGtif, int LineNo, int PixelNo);
...
int main (int argc, char** argv)
{
  ...
  setupGtif (&Ozone_Gtif, OzoneFile);

  // certain valus are assumed...
  Ozone_Gtif.UL[0] = 109.0;  // in decimal degs
  Ozone_Gtif.UL[1] = -7.0;
  Ozone_Gtif.UL[2] = 0.0;

  Ozone_Gtif.LL[0] = 109.0;
  Ozone_Gtif.LL[1] = -47.0;
  Ozone_Gtif.LL[2] = 0.0;

  Ozone_Gtif.UR[0] = 159.0;
  Ozone_Gtif.UR[1] = -7.0;
  Ozone_Gtif.UR[2] = 0.0;

  Ozone_Gtif.LR[0] = 159.0;
  Ozone_Gtif.LR[1] = -47.0;
  Ozone_Gtif.LR[2] = 0.0;

  Ozone_Gtif.scaleX = 2.0;
  Ozone_Gtif.scaleY = 2.0;

  // Check Lat/Long not outside image:
  if ((Lat > Ozone_Gtif.UL[1]) || (Lat < Ozone_Gtif.LR[1])) {
    fprintf (stderr, "ERROR: Lat %f out of range\n", Lat);
    exit(1);
  }
  if ((Long < Ozone_Gtif.UL[0]) || (Long > Ozone_Gtif.LR[0])) {
  ...
  LineNo = (int)( -(Lat - Ozone_Gtif.UL[1])/ Ozone_Gtif.scaleY);
  PixelNo = (int)((Long -  Ozone_Gtif.LL[0])/Ozone_Gtif.scaleY);
  ...
  float OzoneVal = readOzoneValue(&Ozone_Gtif,  LineNo,  PixelNo);

  printf("Line: %d, Pixel: %d, Ozone value: %f \n", LineNo, PixelNo, OzoneVal);


  // Close all open GeoTIFF files before output JPEG image

  closeGtif(&Ozone_Gtif);
...

{
    ...
    tmpBuf = (float *)malloc(inGtif->scanlineSize);
...
const char *mainUsage = "Usage: extractOzone <GeoTIFF_DEM_File> <Lat> <Long>";
